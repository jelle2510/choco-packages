...
$releases = "$domain/gorilla-devs/GDLauncher/releases"
$releases_latest = Get-UrlRedirection "$releases/latest"
$download_latest = $releases_latest -replace "tag", "download"
$specs_yaml = Invoke-WebRequest -Uri "$($download_latest)/latest.yml" -UseBasicParsing
$specs = ConvertFrom-Yaml $specs_yaml

function global:au_GetLatest {
    @{
        URL32 = "$($download_latest)/$($specs.path)"
